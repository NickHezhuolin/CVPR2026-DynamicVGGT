...
def affine_inverse(ext):
    """
    Invert batched rigid transforms with shape (..., 3, 4) or (..., 4, 4).
    Returns camera-to-world matrices when given world-to-camera extrinsics.
    """
    is_torch = isinstance(ext, torch.Tensor)
    if ext.shape[-2:] == (3, 4):
        if is_torch:
            bottom = torch.zeros(ext.shape[:-2] + (1, 4), dtype=ext.dtype, device=ext.device)
            bottom[..., 3] = 1.0
            ext4 = torch.cat([ext, bottom], dim=-2)
        else:
            bottom = np.zeros(ext.shape[:-2] + (1, 4), dtype=ext.dtype)
            bottom[..., 3] = 1.0
            ext4 = np.concatenate([ext, bottom], axis=-2)
    else:
        ext4 = ext

    leading = ext4.shape[:-2]
    flat = ext4.reshape(-1, 4, 4)
    inv = closed_form_inverse_se3(flat)
    return inv.reshape(*leading, 4, 4)
