...
2. Update the following paths with your absolute directory paths:
   ...
   - `resume_checkpoint_path`: Path to your pre-trained VGGT checkpoint (or use `load_pretrained_path` for init-only loading)

## DynamicVGGT Training

For multiview + temporal DynamicVGGT training, use:
...
Edit `training/config/dynamicvggt.yaml`:

- Add dataset entries under `data.train_own_dataset.dataset.dataset_configs`
- Optionally set `checkpoint.load_pretrained_path` to your own initialization weights

Dataset classes must return batches compatible with `MultiComposedDataset` (see `training/data/multi_composed_dataset.py`).
