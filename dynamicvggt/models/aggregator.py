...
class Aggregator(nn.Module):
    ...
    def forward(
        ...
        use_cache=False,
        return_cls_token=False # hzl-edit
        ...
        # Reshape to [B*S, C, H, W] for patch embedding TxV
        ...
        if isinstance(patch_tokens, dict):
            # hezhuolin --- edit
            if return_cls_token:
                cls_tokens = patch_tokens['x_norm_clstoken'] # 取到cls_token
                _, C = cls_tokens.shape
                cls_token_list = cls_tokens.reshape(B, S, C)
        ...
        if return_cls_token:
            return output_list, self.patch_start_idx, cls_token_list
        else:
