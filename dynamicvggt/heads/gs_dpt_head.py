...
class GaussianSplatDPTHead(nn.Module):
    """
    ...
    def __init__(
        ...
        super(GaussianSplatDPTHead, self).__init__()
    ...
    def forward(
        ...
        S = T*V
        images = images.reshape(B, T*V , C_in, H, W)

        motion_token = token_list[-1][:, :, :, :patch_start_idx]

        # Process all frames together if chunk size not specified or large enough
        if frames_chunk_size is None or frames_chunk_size >= S:
            gs, preds, conf = self._forward_impl(token_list, images, patch_start_idx)
            return gs, preds, conf, motion_token
        ...
        if self.is_gsdpt:
            return torch.cat(gs_chunks, dim=1), torch.cat(preds_chunks, dim=1), torch.cat(conf_chunks, dim=1), motion_token
        else:
    ...
    def _forward_impl(
        ...
        for layer_idx in self.intermediate_layer_idx:
            ...
            x = token_list[layer_idx]
            B, T, V, P, C = x.shape
            x = x.reshape(B, T * V, P, C)
            ...
            feat = self.projects[dpt_idx](feat)

            if self.pos_embed:
                feat = self._apply_pos_embed(feat, W, H)
            feat = self.resize_layers[dpt_idx](feat)
            feats.append(feat)
            dpt_idx += 1
